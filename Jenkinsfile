...
// }
    ...
    stage('SecondaryDistros') {
      ...
      steps {
        echo "Am running ${STAGE_NAME}"
        iterateDistros ( "secondary" )
      }
    }
  }
}

def iterateDistros(ofType) {
  ...
  distromap.each { key, value ->
    ...
    if ( value['type'] == ofType ) {
      echo "Running checks for distro:${key}"
      testrunner(key, value)
    }
  }
}
...
def testrunner(distroName, distroMap) {
  ...
  stage(distroName) {
    ...
    testmap.each { key, value ->
      ...
      stage(key['name']) {
        // We are now in 'scripted' mode, so no 'agent', we use node() to
        // place the job
        node(distroName) {
          script {
            sh "${key['commands']}"
